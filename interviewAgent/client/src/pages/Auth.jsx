...
const Auth = () => {
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      if (code === "auth/popup-closed-by-user") {
        ...
        setErrorMsg("Domain not authorized in Firebase Console. Use 1-Click Demo Login or Email Login below.");
      } else {
  ...
  const handleDemoAuth = async () => {
    try {
      ...
      setErrorMsg(error.response?.data?.message || "Demo login failed. Make sure backend server is running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEmailAuth = async (e) => {
    ...
    try {
      ...
      setErrorMsg(error.response?.data?.message || "Email login failed. Make sure backend server is running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full min-h-screen bg-[#f6f7fb] flex flex-col items-center justify-center px-4 py-12 selection:bg-black selection:text-white relative">
      <button
        onClick={() => navigate("/")}
        className="absolute top-6 left-6 flex items-center gap-2 text-xs font-bold text-gray-500 hover:text-gray-900 transition"
      >
        <FaArrowLeft size={12} />
      ...
      >
        ...
        <div className="text-center mb-6 space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-50 text-emerald-700 text-xs font-bold rounded-full">
            <IoSparkles size={14} />
            <span>AI Mock Interview Suite</span>
          </div>
          ...
          <p className="text-gray-500 text-xs md:text-sm leading-relaxed">
            Start role-specific mock interviews, track performance scores, and unlock detailed coaching reports.
          </p>
        </div>
        ...
        {authMode === "options" ? (
          <div className="space-y-3">
            ...
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-2.5 py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl font-bold shadow-lg shadow-emerald-600/20 transition text-sm cursor-pointer"
            >
              <FaUserCheck size={16} />
            ...
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-3 py-3.5 bg-white hover:bg-gray-50 text-gray-800 border-2 border-gray-200 rounded-2xl font-semibold shadow-xs transition text-sm cursor-pointer"
            >
              <FcGoogle size={20} />
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-2xl font-semibold transition text-xs cursor-pointer"
            >
        ...
        <p className="text-center text-[11px] text-gray-400 mt-6 leading-relaxed">
          Includes 200 free trial credits for new accounts to start practicing immediately.
        </p>
      </motion.div>
    </div>
  );
